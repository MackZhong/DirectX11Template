...
bool Game::ResizeSwapChain( int width, int height )
{
    ...
    if ( FAILED( hr ) )
    {
        MessageBoxW( m_Window.get_WindowHandle(), L"Failed to retrieve the swap chain back buffer.", L"Error", MB_OK|MB_ICONERROR );
        return false;
    }

    hr = m_d3dDevice->CreateRenderTargetView( backBuffer.Get(), nullptr, m_d3dRenderTargetView.ReleaseAndGetAddressOf() );
    if ( FAILED( hr ) )
    {
        MessageBoxW( m_Window.get_WindowHandle(), L"Failed to create the RenderTargetView.", L"Error", MB_OK|MB_ICONERROR );
        return false;
    }
    ...
    hr = m_d3dDevice->CreateTexture2D( &depthStencilBufferDesc, nullptr, m_d3dDepthStencilBuffer.ReleaseAndGetAddressOf() );
    if ( FAILED(hr) )
    {
        MessageBoxW( m_Window.get_WindowHandle(), L"Failed to create the Depth/Stencil texture.", L"Error", MB_OK|MB_ICONERROR );
        return false;
    }

    hr = m_d3dDevice->CreateDepthStencilView( m_d3dDepthStencilBuffer.Get(), nullptr, m_d3dDepthStencilView.ReleaseAndGetAddressOf() );
    if ( FAILED(hr) )
    {
        MessageBoxW( m_Window.get_WindowHandle(), L"Failed to create DepthStencilView.", L"Error", MB_OK|MB_ICONERROR );
        return false;
    }
...
bool Game::Initialize()
{
    ...
    {
        MessageBoxW( m_Window.get_WindowHandle(), L"Failed to verify DirectX Math library support.", L"Error", MB_OK|MB_ICONERROR );
        return false;
    }
    ...
    if ( FAILED(hr) )
    {
        MessageBoxW( m_Window.get_WindowHandle(), L"Failed to create DirectX 11 Device.", L"Error", MB_OK|MB_ICONERROR );
        return false;
    }
    ...
    if ( FAILED(hr) )
    {
        MessageBoxW( m_Window.get_WindowHandle(), L"Failed to create IDXGIFactory2.", L"Error", MB_OK|MB_ICONERROR );
        return false;
    }
    ...
    if ( FAILED(hr) )
    {
        MessageBoxW( m_Window.get_WindowHandle(), L"Failed to create swap chain.", L"Error", MB_OK|MB_ICONERROR );
        return false;
    }

    if ( !ResizeSwapChain( m_Window.get_ClientWidth(), m_Window.get_ClientHeight() ) )
    {
        MessageBoxW( m_Window.get_WindowHandle(), L"Failed to resize the swap chain.", L"Error", MB_OK|MB_ICONERROR );
        return false;
    }
    ...
    hr = m_d3dDevice->CreateDepthStencilState( &depthStencilStateDesc, m_d3dDepthStencilState.ReleaseAndGetAddressOf() );
    if ( FAILED(hr) )
    {
        MessageBoxW( m_Window.get_WindowHandle(), L"Failed to create a DepthStencilState object.", L"Error", MB_OK|MB_ICONERROR );
        return false;
    }
    ...
    hr = m_d3dDevice->CreateRasterizerState( &rasterizerDesc, m_d3dRasterizerState.ReleaseAndGetAddressOf() );
    if ( FAILED(hr) )
    {
        MessageBoxW( m_Window.get_WindowHandle(), L"Failed to create a RasterizerState object.", L"Error", MB_OK|MB_ICONERROR );
        return false;
    }
