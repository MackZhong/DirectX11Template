...
typedef std::map< std::wstring, Window* > WindowNameMap;
	...
	: m_hInstance(hInst)
{
	...
	{
		MessageBoxW(NULL, L"Unable to register the window class.", L"Error", MB_OK | MB_ICONERROR);
	}
}
...
Window& Application::CreateRenderWindow(const std::wstring& windowName, int clientWidth, int clientHeight, bool vSync, bool windowed)
{
	...
	if (!hWnd)
	{
		MessageBoxW(NULL, L"Could not create the render window.", L"Error", MB_OK | MB_ICONERROR);
